...
ax1.plot(xJ,γJ,"--",color=medblue)
ax1.plot(xJ,γJ4,"-.",color="w")
ax1.set_xticks([0,0.2,0.4])
ax1.set_xlim([0,0.55]); ax1.set_ylim([0,0.55])
ax1.set_ylabel(L"$J/\hbar\omega_r$",fontsize=10)
...
ax2.plot(Xftub,Yftub,color=darkred)
ax2.plot(xJ,γJ,"--",color=medblue)
ax2.plot(xJ,γJ4,"-.",color="w")
...
cb.ax.set_position([pos.x0-0.075, pos.y0, pos.width, pos.height]) # shift colorbar slightly to the left
cb.ax.set_xlabel(L"$\Delta N/N_f$", fontsize=10) # put label to the right of the bar
...
display(fig); fig.savefig(joinpath(@__DIR__,"PhaseDiagram_L70.pdf"),format="pdf",bbox_inches="tight")
